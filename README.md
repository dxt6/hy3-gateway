...
6. **上游瞬时错误重试 + 并发控制**：命中可重试的瞬时错误（429/限流，**以及**上游 5xx、502/500/503/504、`network` 类、JSON-RPC `code:-32603/-32001` 内部/网络错误、`category:internal/network`）指数退避重试 3 次（1s/2s/4s）；**上游即便返回 HTTP 200 + 错误体（如 `{code:-32603,message:"Internal error"}`）也会当作失败重试**；明确的请求/参数错误（4xx、invalid、too long）不重试。重试前后打印 `[hermes][UPSTREAM-ERR]/[UPSTREAM-EXC]` 日志，便于直接定位上游真因。并发信号量默认 4，排队不报错。
