...
| 08-13 22:34 | **WS "Connection reset without closing handshake" 修复 + WS 日志打全** | 现象：拆了 GPT/cockpit 等后 codex 报 `stream disconnected before completion: WebSocket protocol error: Connection reset without closing handshake`。排查：服务端日志此前 WS 的 `error/close` 都是空处理，连接被悄悄掐掉不留痕；而 `flush()` 把内部 SSE 的 `: ping` 保活注释丢弃，导致 **WS 在长 prefill（527 条上下文可达数十秒）期间零字节流向 codex**，公网链路 NAT/防火墙 idle 超时即可 RST。修复：① 应用层保活——`handleWsResponses` 每 15s 向客户端发 `ws.ping()`，且把内部 `: ping` 注释实时转成 `ws.ping()` 帧，重置 idle 计时；② `server.timeout=0` + 升级时 `ws._socket.setTimeout(0)` 关掉 Node 默认 2 分钟 socket 超时；③ WS 日志打全（OPEN/MSG/REQ/UP status/UP end frames/CLOSE code+reason/WS-ERR 带 stack），并加 `uncaughtException`/`unhandledRejection` 全局兜底日志。部署 md5 d843601d，本地 WS 实测 12 帧 completed=true 且 `[hermes][WS]` 全链路日志可见 |
| 08-13 22:45 | **codex "一直思考卡住" 修复（流停滞看门狗）** | 现象：上一轮加 ping 保活后，连接不再 RST，但变"无限转圈"——codex 请求 `CHAT`（如 msgs=471/472）只收到请求、**无 `STREAM_DONE` 也无报错**，上游（CloudBase/hy3）收了请求却流挂起。根因：网关对流式无"卡死超时"，上游一挂起，保活让连接活着 → codex 永远等。修复：`handleResponses` 流式分支加**流停滞看门狗**——每收到一条上游 `data:` 就刷新时间戳；每 5s 检查，若超过 `STALL_MS=150000` 无任何上游 token，则 `reader.cancel()` 取消上游并发 `response.completed(status='failed', error='upstream stream stalled')`，让 codex 停止并触发其自身重试，而非死等。正常流（毫秒级 delta）不受影响。部署 md5 25b42a1d，实测 13 帧 completed=1 且未误触发 STALL。注：若仍卡，优先看 `journalctl -u hermes-proxy` 最新 `CHAT` 是否配 `STREAM_DONE`；有 `STALL abort` 即看门狗生效，等 codex 重试即可 |
| 08-13 22:57 | **【治本】旧对话必卡 / 新对话没事——根因=超长上下文上游静默挂起** | 现象：用户报告"旧对话（上下文大）卡死思考，新开对话正常"。日志铁证：旧对话请求 `CHAT msgs=479` 只有 `CHAT`、**无 `STREAM_DONE`** 上游静默挂起；新对话 `msgs` 小（1~25）全部 `completed=1` 秒回。根因：`responsesToChat` 原有上下文安全阀 `HARD_CTX_EST=250000` 灰度太大——旧对话约 188K 估算 token 远低于阈值，**上下文保护从未触发**，上游 hy3 在 ~80-190K 上下文上随机静默挂起（不结束不报错）。修复（治本）：`trimContext` 加**软阈值裁剪** `SOFT_CTX_EST=40000` 估计 token——超过且消息数>`KEEP_CTX_MSGS=60` 时，从头部成对删最旧轮次、保留最近 60 条，并打 `[hermes][WARN] context SOFT-TRIM dropped N oldest msgs, kept last 60` 日志；看门狗 `STALL_MS` 同步降到 90000 作更快兜底。实测：伪造 `nMsgs=479` 旧对话请求经网关 → 日志 `SOFT-TRIM dropped 420 oldest msgs (est 188603→29681 tok)` 后**每次都 `completed=1` 且 ~1.5s 返回**（此前必卡）；`nMsgs=20` 小请求不触发 TRIM。已部署 md5 7e264458。经验：上游 hy3 对超长上下文稳定性差，网关侧主动保留最近 N 条是抗挂起最稳手段；若某模型上下文更长可上调 KEEP_CTX_MSGS |
| 08-13 23:18 | **上下文软裁剪升级为"摘要压缩"代替整段删除（更不丢信息）** | 用户质疑"整段删旧历史"太粗暴，要求更优解。说明没有十全十美（瓶颈在上游 hy3 无法处理 >~80K 上下文），但"让模型总结大上下文"走不通（总结本身也需喂大上下文→同样挂起）。改进：超出 `SOFT_CTX_EST` 时**不再整段删**，而是把被砍的旧消息确定性地提炼成"工具调用轨迹"摘要（`buildCompactSummary`：提取 function_call/custom_tool_call + tool_result 配对为 `name → 结果摘要`，不调上游），作为一条 `system` 笔记**前置注入**上下文，保留最近 60 条。`COMPACT` 日志：`context COMPACT dropped 419 oldest msgs (est 246089→30873 tok), kept last 61, injected summary N chars`。对 codex 类写代码 agent，早期信息以工具轨迹为主，摘要远比"丢裸历史"有用。实测：伪造 `nMsgs=479` 文本对 → `COMPACT dropped 419 (est 246089→30873 tok)` 且 BIG 请求 2147ms `completed=true`；摘要在真实 codex 旧对话里会保留 function_call/tool_result 轨迹。部署 md5 c8b9834f。注：纯文本对话的摘要价值有限（仅提示语），但"卡死 vs 丢早期原文"二选一时，压缩保活优于硬删；若需零损失只能 `/compact` 由用户手动或换长上下文模型 |
